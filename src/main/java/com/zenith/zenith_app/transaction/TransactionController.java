...
public class TransactionController {
    ...
    @PostMapping("/new")
    public TransactionDTO createTransaction(@RequestBody CreateTransactionRequest transactionRequest,
                                                @AuthenticationPrincipal UserDetails userDetails){
        return transactionService.createTransaction(transactionRequest, userDetails.getUsername());
    }

    @PutMapping("/update/{id}")
    public TransactionDTO updateTransaction(@RequestBody UpdateTransactionRequest updateTransactionRequest,
                                            @PathVariable Long id,
                                            @AuthenticationPrincipal UserDetails userDetails){
        return transactionService.updateTransaction(updateTransactionRequest, id, userDetails.getUsername());
    }

    @GetMapping("/view/all")
    public List<TransactionDTO> viewAllTransactions(@AuthenticationPrincipal UserDetails userDetails){
        return transactionService.viewAllTransactions(userDetails.getUsername());
    }

    @GetMapping("/view/id/{id}")
    public TransactionDTO viewTransactionById(@PathVariable Long id, @AuthenticationPrincipal UserDetails userDetails){
        return transactionService.viewTransactionById(id, userDetails.getUsername());
    }

    @GetMapping("/view/name/{transactionName}")
    public List<TransactionDTO> viewTransactionByName(@PathVariable String transactionName,
                                                      @AuthenticationPrincipal UserDetails userDetails){
