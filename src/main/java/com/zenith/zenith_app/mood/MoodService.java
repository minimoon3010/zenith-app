...
public class MoodService {
    ...
    public List<MoodDTO> viewAllMood(String username){
        return moodRepository.findByUser_Username(username)
                .stream()
    ...
    public List<MoodDTO> filterMoodByStatus(Integer status, String username){
        return moodRepository.findByStatusAndUser_Username(status, username)
                .stream()
                .map(MoodDTO::fromMood)
                .collect(Collectors.toList());
    }

    public List<MoodDTO> filterByTimestamp(LocalDateTime earlier, LocalDateTime later, String username){
        return moodRepository.findByLastUpdatedBetweenAndUser_Username(earlier, later, username)
                .stream()
                .map(MoodDTO::fromMood)
                .collect(Collectors.toList());
    }
    ...
    private Mood getSecureMood(Long id, String username){
        Mood mood = moodRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Mood does not exist."));
