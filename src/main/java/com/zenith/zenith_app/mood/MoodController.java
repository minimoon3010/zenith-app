...
public class MoodController {
    ...
    @PostMapping("/new")
    public MoodDTO createMood(@Valid @RequestBody CreateMoodRequest request,
                              @AuthenticationPrincipal UserDetails userDetails) {
        return moodService.createMood(request, userDetails.getUsername());
    }

    @PutMapping("/update/{id}")
    public MoodDTO updateMood(@Valid @RequestBody UpdateMoodRequest request,
    ...
    public MoodDTO viewMoodById(@PathVariable Long id,
                                @AuthenticationPrincipal UserDetails userDetails){
        return moodService.viewMoodById(id, userDetails.getUsername());
    }

    @GetMapping("/filter/status")
    public List<MoodDTO> filterMoodByStatus(@RequestParam Integer status,
                                            @AuthenticationPrincipal UserDetails userDetails){
        return moodService.filterMoodByStatus(status, userDetails.getUsername());
    }

    @GetMapping("/filter/time")
    public List<MoodDTO> filterMoodByTime(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime earlier,
