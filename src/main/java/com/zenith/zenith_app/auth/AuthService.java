...
public class AuthService {
    ...
    public JWTResponse login(LoginRequest loginRequest) {
        ...
        if (passwordEncoder.matches(loginRequest.password(), userFound.getPassword())) {
            return new JWTResponse(jwtUtil.generateToken(userFound.getUsername()));
        } else {
    ...
    private User findUserByIdentifier(LoginRequest loginRequest){
        ...
        return (email != null ? userRepository.findByEmail(email) : userRepository.findByUsername(username))
